package com.dnapayments.presentation.sdulife.news.founds.view.adapter


import android.widget.TextView
import com.bumptech.glide.Glide
import com.bumptech.glide.request.RequestOptions
import com.dnapayments.R
import com.dnapayments.data.model.Founds
import com.dnapayments.utils.base.BaseAdapter
import com.dnapayments.utils.base.ViewHolder


class FoundsAdapter(private val myCallBack: (result: Int) -> Unit) :
    BaseAdapter<Founds>(R.layout.item_founds) {
    override fun bindViewHolder(holder: ViewHolder, data: Founds) {
        holder.itemView.apply {
            findViewById<TextView>(R.id.title).text = data.title
            findViewById<TextView>(R.id.tv_hour).text = data.hour
            findViewById<TextView>(R.id.tv_date).text = data.date
            Glide.with(holder.itemView.context)
                .load(data.img)
                .apply(
                    RequestOptions.placeholderOf(R.drawable.ic_document_placeholder)
                        .dontAnimate().error(R.drawable.ic_document_placeholder)
                )
                .into(findViewById(R.id.iv_founds))

            setOnClickListener {
                myCallBack.invoke(data.found_id)
            }
        }
    }

}